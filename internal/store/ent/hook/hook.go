...
// The ClusterSettingFunc type is an adapter to allow the use of ordinary
// function as ClusterSetting mutator.
type ClusterSettingFunc func(context.Context, *ent.ClusterSettingMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ClusterSettingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ClusterSettingMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ClusterSettingMutation", m)
}
